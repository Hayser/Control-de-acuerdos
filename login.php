...
if (isset($_POST['login'])) {
    ...
    if ($obj->Normal_Query("SELECT * FROM users WHERE email = ?", [$email])) {
        if ($obj->Count_Rows() == 0) {
            header("location: ../MuniV5.1/login.php?login=email");
            exit();
        } else {
            ...
            $user_rol = $row->rolUsuario;
            $user_deparment = $row->departamento;
            if (password_verify($password, $db_password)) {
                ...
                $obj->Create_Session("user_rol", $user_rol);
                $obj->Create_Session("user_deparment", $user_deparment);
                header("location:dash.php");
            } else {
                header("location: ../MuniV5.1/login.php?login=password");
                exit();
            }
        }
    }
...
<body>
    <div class="container">
        <div class="header clearfix">
            ...
            <h4 class="text-muted" style>Ingreso al sistema MSIH</h4>
        </div>
